...
peace_wordcloudcolored <- wordcloud(names(freq), freq, max.words=100, rot.per=0.2, colors=dark2)
...
### peace by decade
peace_word_frequencies_topfive %>% ggvis(~years, ~frequency_war) %>% layer_lines(stroke = 'war') %>% add_axis("x", title = "Importance of words in nobel prize for peace speeches(by decades)")    %>% add_axis("y", title = "Word frequency") %>% layer_paths(y = ~frequency_world, stroke = 'world') %>% layer_paths(y = ~frequency_peace, stroke = 'peace') %>% layer_paths(y = ~frequency_international, stroke = 'international') %>% layer_paths(y = ~frequency_one, stroke = 'one')
